...
class Gridworld:
    ...
    def __init__(self, width, height, start_pos, goal_pos_list, num_rewards, num_traps,
                 ...
                 initial_agent_points=0): # NEU: initial_agent_points hinzugefügt
        self.width = width
        self.height = height
        # Das Gitter speichert den Typ der Zelle (0: leer, 1: Start, 2: Ziel, 3: Falle, 4: Belohnung)
        ...
        # WICHTIG: Eine Wahrnehmung der *gesamten* Karte ist mit einer Q-Tabelle nicht praktikabel,
        # da der Zustandsraum exponentiell explodieren würde (z.B. 3^25 für eine 5x5 Karte).
        # Für eine solche globale Wahrnehmung müsste man Deep Reinforcement Learning (z.B. mit CNNs) verwenden.
        # Daher bleibt die Zustandsdefinition auf die Position des Agenten + seine direkten Nachbarn beschränkt.
        self.NUM_NEIGHBOR_TYPES = 3 # EMPTY/START/GOAL -> 0, REWARD -> 1, TRAP -> 2
        self.num_states = (self.width * self.height) * (self.NUM_NEIGHBOR_TYPES ** 4)
        ...
        self.num_actions = len(self.ACTIONS) # Anzahl der möglichen Aktionen
        ...
        self.INITIAL_AGENT_POINTS = initial_agent_points # NEU: Startpunkte des Agenten

        # FIX: _initialize_grid muss einmal aufgerufen werden, um _base_grid_state zu setzen
        self._initialize_grid() 
        # reset() wird dann aufgerufen, um die erste Gridworld mit zufälligen Elementen zu füllen
        self.reset() 
    ...
    def reset(self):
        ...
        # NEU: Rückgabe des erweiterten Startzustands
    ...
    def get_state(self, agent_x, agent_y):
        """
        Konvertiert Agenten-Koordinaten und Umgebungsdetails in einen einzelnen Zustands-Index.
        ...
        # Kombiniere die Nachbar-Typen zu einem einzelnen Index
        # Beispiel: up_type * 3^0 + down_type * 3^1 + left_type * 3^2 + right_type * 3^3
        # Hier 3, da es 3 mögliche Typen für jeden Nachbarn gibt (0, 1, 2)
        neighbor_encoding = 0
        ...
        # Die Nachbar-Kodierung wird um den Faktor (width*height) verschoben,
        # um sicherzustellen, dass jeder Zustand eindeutig ist.
        # Der maximale Wert für pos_state ist (width*height - 1).
        # Der maximale Wert für neighbor_encoding ist (2 * 3^0) + (2 * 3^1) + (2 * 3^2) + (2 * 3^3) = 2 + 6 + 18 + 54 = 80
        # Total states = (width*height) * (NUM_NEIGHBOR_TYPES^4)
        # Der State Index ist: pos_state + neighbor_encoding * (self.width * self.height)
        # Dies ist eine gängige Methode, um mehrere diskrete Werte in einen einzelnen Index zu packen.
        state = pos_state + (neighbor_encoding * (self.width * self.height))
    ...
    def get_coords_from_state(self, state_index):
        ...
        Beachten Sie, dass diese Funktion jetzt nur die Position aus dem erweiterten Zustand extrahiert.
        Die Nachbarinformationen gehen hierbei verloren, was für die Visualisierung in Ordnung ist.
        """
    ...
    def step(self, current_state_index, action):
        ...
        # NEU: Der nächste Zustand wird mit der erweiterten Zustandsdefinition berechnet
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                elif event.type == pygame.KEYDOWN: # Korrektur: event.type muss pygame.KEYDOWN sein
...
if __name__ == "__main__":
    ...
    GRID_WIDTH = 5 
    GRID_HEIGHT = 5 
    START_COORDINATES = (0, GRID_HEIGHT - 1) # Unten links
    GOAL_COORDINATES = [(GRID_WIDTH - 1, 0)] # Oben rechts
    NUM_REWARDS_ON_MAP = 4 
    NUM_TRAPS_ON_MAP = 2 

    # Belohnungswerte (manuell anpassbar)
    REWARD_GOAL_VALUE = 500        # Belohnung für das Erreichen eines Ziels (niedriger als max Belohnungen)
    REWARD_REGULAR_STEP_VALUE = -1 # Geringe Schrittkosten
    REWARD_TRAP_VALUE = -10000     # EXTREM hoher Abzug für Fallen, MUSS vermieden werden!
    REWARD_COLLECT_VALUE = 1000    # Sehr hohe Belohnung für das Einsammeln
    INITIAL_AGENT_POINTS = 0       # Startpunkte des Agenten

    # --- Konfigurierbare Parameter für Q-Learning Agent ---
    LEARNING_RATE = 0.1            # Alpha (α)
    DISCOUNT_FACTOR = 0.99         # Gamma (γ) - Sehr weitsichtig
    EPSILON_START = 1.0            # Startwert für Epsilon (100% Exploration)
    MIN_EPSILON = 0.01             # Minimaler Epsilon-Wert
    
    NUM_EPISODES = 1000000         # Anzahl der Trainingsepisoden (Beibehalten für robuste Lernkurve)
    EPSILON_DECAY_RATE = (EPSILON_START - MIN_EPSILON) / NUM_EPISODES 
    
    MAX_STEPS_PER_EPISODE = 300   # Maximale Schritte pro Episode, großzügig für adaptive Pfade
    
    # Evaluierungs-Epsilon auf 0.0 setzen, da die adaptive Verhaltensweise aus der gelernten Policy kommen soll
    MIN_EPSILON_EVALUATION = 0.0 # Keine Exploration in der Evaluierung

    # Visualisierungseinstellungen
    VISUALIZE_EVERY_N_EPISODES = 100000 # Alle X Episoden visualisieren
    ...
    Q_TABLE_FILENAME = "trained_q_table.npy"
    ...
    TRAIN_MODE = True # Ändere dies zu False, um den Agenten zu laden und zu testen
    ...
    if TRAIN_MODE:
        ...
        for episode in range(1, NUM_EPISODES + 1):
            ...
            if episode % 1000 == 0: # Ausgabe seltener für 1M Episoden
    ...
    # Epsilon für die Evaluierung auf 0.0 setzen, damit der Agent nur das Gelernte ausnutzt.
    # Die adaptive Verhaltensweise soll aus der gelernten Q-Tabelle kommen, nicht aus Exploration.
